  ...
  changelog: [
    {
      ...
      type: "added",
      items: [
        "Added AM/PM support. To use, add `am` or `pm` to the end of the timestamp.",
      ],
    },
  ],
};
      ...
      return class ReplaceTimestamps extends Plugin {
        ...
        patchMessage() {
          Patcher.before(
            ...
            (_, [, msg]) => {
              const regexAGlobal =
                /(?<!\d)\d{1,2}:\d{2}(?!\d)(am|pm)?(t|T|d|D|f|F|R)?/gi;
              const regexA =
                /((?<!\d)\d{1,2}:\d{2}(?!\d))(am|pm)?(t|T|d|D|f|F|R)?/i;
              if (msg.content.search(regexAGlobal) !== -1)
                msg.content = msg.content.replace(regexAGlobal, (x) => {
                  ...
                  if (
                    ...
                    (mode && mode.toLowerCase() === "am" && hours === 12) ||
                    hours === 24
                  )
                    time = `00:${minutes}`;
                  else if (minutes >= 60) {
